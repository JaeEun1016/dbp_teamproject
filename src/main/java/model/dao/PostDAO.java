...
public class PostDAO {
    ...
    public int addPostWithTasks(Post post) throws SQLException {
        ...
        String taskSql = "INSERT INTO tasks (taskId, post_id, user_id, order, description, isCompleted, taskDate) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            // Task insert 실행
            for (Task task : post.getTasks()) {
                Object[] taskParams = { task.getTaskId(), post.getPostId(), task.getUserId(), task.getOrder(), task.getDescription(), task.isCompleted(), new java.sql.Date(task.getTaskDate().getTime()) };
        ...
        try {
            ...
            if (post != null) {
                ...
                while (taskRs.next()) {
                    ...
                    task.setPostId(taskRs.getInt("post_id"));
                    task.setUserId(taskRs.getInt("user_id"));
                    ...
                    task.setTaskDate(taskRs.getDate("taskDate"));
                    tasks.add(task);
                }
                taskRs.close();
                post.setTasks(tasks);
    ...
    public int updatePostWithTasks(Post post) throws SQLException {
        ...
        String insertTaskSql = "INSERT INTO tasks (taskId, post_id, user_id, order, description, isCompleted, taskDate) VALUES (?, ?, ?, ?, ?, ?, ?)";
